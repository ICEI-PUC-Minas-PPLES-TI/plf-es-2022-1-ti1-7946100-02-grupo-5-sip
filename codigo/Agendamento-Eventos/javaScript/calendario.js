...
const reenderizarCalendario = () =>{
    ...
    document.querySelector(".data p").innerHTML= new Date().toDateString(); //linka a data atual 
...
const text = document.getElementById("inputDescricao");
...
window.onclick = function(event) {
    if (!event.target.matches('.menu' || 'caixaBotao')) { 
...
function novaData(click){
    ...
    if(opcaoClicada=="Reunião"){
        ...
        click.target.innerHTML += text.value;
        text.value = '';
        //document.getElementsByClassName('reuniãoCriada').style.backgroundColor = 'Red';
    }
    else{
